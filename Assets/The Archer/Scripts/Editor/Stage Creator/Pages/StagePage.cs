...
namespace OctoberStudio.StageCreator
{
    public class StagePage
    {
        ...
        protected virtual void OnRoomSelected(RoomListItem selectedItem, RoomListItem prevSelectedItem)
        {
            if (prevSelectedItem != selectedItem)
            {
                ...
                if (RoomPrefabsHandler != null)
                {
                    if (RoomPrefabsHandler.IsRoomLoaded)
                    {
                        ...
                        {
                            var prevPrefabsProperty = prevSelectedItem.RoomProperty.FindPropertyRelative("prefabs");
                            RoomPrefabsHandler.SavePrefabs(prevPrefabsProperty);
                        }

                        RoomPrefabsHandler.DeletePrefabs(false);
                    }
                    var prefabsProperty = selectedItem.RoomProperty.FindPropertyRelative("prefabs");
                    RoomPrefabsHandler.LoadPrefabs(prefabsProperty);
                }
                ...
                if (WavesListContainer != null)
                {
                    ...
                    WavesListContainer = new WavesListContainer(selectedItem.RoomProperty, 0);
        ...
        public virtual int CalculateGold(SerializedProperty currentWave)
        {
            var gold = 0;
            ...
            {
                var roomItem = RoomsListContainer.RoomsListView[i];
                gold += roomItem.GetAllGold();
            }

            gold += RoomsListContainer.RoomsListView.SelectedItem.GetAllGoldUpTo(currentWave);
